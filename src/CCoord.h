...
public:
    ...
    /** Operators + and - */
    CCoord operator + (const CCoord & other)const { return CCoord(this->m_X + other.m_X, this->m_Y + other.m_Y);}
    CCoord operator - (const CCoord & other) const { return CCoord(this->m_X - other.m_X, this->m_Y - other.m_Y);}
    friend std::ostream & operator << (std::ostream & ostream, const CCoord & coord) {ostream << "(" << coord.m_X << ", " << coord.m_Y << ")"; return ostream;}
    /**
     * Compares two points.
     * @param other
